...
- **Priority:** HIGH (6.6.3–6.6.4 still block real 3D work; the rest are papercuts)
- **Status:** 6.6.1/6.6.2 landed 2026-09-01 — the script path now returns output, results and errors, so the `execute_script` + PowerShell-file-read workaround is no longer needed for data. Seeing the frame (6.6.3) and reading errors (6.6.4) are still routed around.
...
**Last Updated:** 2026-09-01 (6.6.1 + 6.6.2 fixed: `execute_script` now returns `output`/`errors`/`result` and finishes coroutines under a `timeout` watchdog; body runs as `_mcp_body()` because 4.7 rejects a value returned from the `void` virtual `_run()`. Full E2E green on both engines: **201 passed / 0 failed, coverage 163/163**, verified on 4.7.2 and 4.4.1. Next per plan: 6.6.3 (render through the game camera), 6.6.4 (`get_error_log`), then 6.6.5.)
